...
IntelliQuery AI is a production-ready **planner-based agentic system** with 4 main capabilities:
...
4. **Autonomous Planning**: Goal-driven multi-step reasoning agent
...
│  /ask-agentic      /agent/plan   /agent/tools                │
...
│  │  Agent      │  │  (State Mgmt)│  │  (Insights)     │     │
│  └─────────────┘  └──────────────┘  └─────────────────┘     │
│                           │                                   │
│                    Tool Registry                              │
│   ┌──────────────────────┬───────────────────────┐          │
│   │  RAG Tools      SQL Tools      ML Tools      │          │
│   │  Chart Tools    Utility Tools                │          │
│   └──────────────────────────────────────────────┘          │
└──────────────────────────┬──────────────────────────────────┘
                           │
        ┌──────────────────┼──────────────────┐
...
┌──────────────┐  ┌──────────────┐  ┌──────────────┐
│ rag/         │  │ analytics/   │  │ ml/          │
│ document_    │  │ text_to_sql  │  │ predictor    │
│ processor    │  │ data_handler │  │ (ML)         │
└──────┬───────┘  └──────┬───────┘  └──────┬───────┘
       │                 │                  │
       └─────────────────┴──────────────────┘
                       │
...
┌─────────────────────────────────────────────────────────────┐
│                    PLANNER AGENT                             │
│  - Interprets goal as a multi-step analysis task            │
...
│  - model_metrics: {accuracy, features...}                    │
│  - charts: [base64 images]                                   │
│  - insights: ["model trained...", "key feature..."]          │
└──────────────────────────┬──────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────┐
...
|------|----------|-------------|
| `rag_search` | RAG | Search documents for information |
| `rag_answer` | RAG | Answer questions using RAG |
| `sql_query` | SQL | Execute natural language data queries |
| `sql_schema` | SQL | Get table schema |
| `data_stats` | SQL | Get data statistics |
| `ml_train` | ML | Train classification model |
| `ml_predict` | ML | Single prediction |
| `ml_batch_predict` | ML | Batch predictions |
| `ml_feature_importance` | ML | Get feature importance |
| `chart_distribution` | Viz | Churn distribution chart |
| `chart_by_category` | Viz | Category breakdown chart |
| `chart_feature_importance` | Viz | Feature importance chart |
